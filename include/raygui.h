...
bool GuiTextBox(Rectangle bounds, char *text, int textSize, bool editMode)
{
    ...
    // DrawText(text, textBounds.x, textBounds.y, 20, Fade(GetColor(GuiGetStyle(TEXTBOX, TEXT + (state*3))), guiAlpha));
    // cant use drawtext need change cursor accordingly
...
static void GuiDrawText(const char *text, Rectangle bounds, int alignment, Color tint)
{
    ...
    if ((text != NULL) && (text[0] != '\0'))
    {
        ...
        int lineCount = 0;
        char **lines = GetTextLines((char*)text, &lineCount);
        ...
        {
            int iconId = 0;
            lines[i] = (char*)GetTextIcon(lines[i], &iconId);      // Check text for icon and move cursor
